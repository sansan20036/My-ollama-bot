...
class ChatService:
    def __init__(self):
        ...
        # 修改：將預設模型切換為支援視覺的 gemma3:27b
        target_model = "gemma3:26b"
    ...
    async def process_query(self, query: str, history: list, images: list = None) -> AsyncGenerator[str, None]:
        # ✨ 新增全域防護罩：包住整個對話流程
        try:
            real_query = query
            ...
            history_text = "\n".join([f"{msg['role']}: {msg.get('content', '')}" for msg in history[-2:]]) if history else "(無歷史紀錄)"
                    ...
                    if df is not None and not df.empty:
                        ...
                        try:
                            ...
                            code_prompt = (
                                ...
                                f"4. 【極度重要】如果要對某個欄位進行加總 (sum) 或數學運算，該欄位可能混雜中文字，請務必先使用 `pd.to_numeric(..., errors='coerce')` 轉換型別！例如：pd.to_numeric(df['時數'], errors='coerce').sum()\n"
            ...
            if matches:
                ...
                for m in matches:
                    ...
                    if not is_snipe_success:
                        print(f"狙擊失敗：在 {sniper_k} 筆切片中，完全找不到包含 '{target_id}' 的精確內容。")
            ...
            def final_rank(doc):
                ...
                # 修改：檔名精準命中霸王條款
                # 如果使用者問的問題裡面，剛好包含了這個資料塊的檔名，直接加 5000 分！
                if source_name and source_name in real_query:
                    score += 5000
